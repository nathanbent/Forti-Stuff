    ...
    with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
        for index, ip in enumerate(infile, start=1):
            ...
            if ip:  # Make sure the line is not empty
                outfile.write(f"edit SAP-{index}\n")
                outfile.write(f"set subnet {ip}/32\n")
                outfile.write("next\n\n")

if __name__ == "__main__":
    input_file = "ip_list.txt"  # Replace with your input filename
    output_file = "fortigate_formatted_ips.txt"  # Replace with your desired output filename
